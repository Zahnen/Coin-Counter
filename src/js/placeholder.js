...
const coinCounter = (moneyToCount, coinCount) => {
  ...
  if (moneyToCount <= 0) {
    ...
    coinValues.shift();
    return coinCounter(moneyToCount, coinCount);
  }
}
...
// REFACTOR
// Analysis:
// 1. We always get the same output; it only works once - NOT DONE
// 2. Always returns something - DONE
// 3. Cannot rely on external variables - NOT DONE
// 4. Cannot rely on external state - ? don't think we have state
// 5. No side effects - ? not sure
